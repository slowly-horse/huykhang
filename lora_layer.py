...
class Linear(nn.Linear, LoraLayer):
    ...
    def merge(self):
        ...
        if self.r[self.active_adapter] > 0:
            ...
            lora_update = self.lora_B[self.active_adapter].weight @ self.lora_A[self.active_adapter].weight
            lora_update = transpose(lora_update, self.fan_in_fan_out)
            
            # to the original weights, scaled by the LoRA scaling factor
            self.weight.data += lora_update * self.scaling[self.active_adapter]
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            after_A = self.lora_A[self.active_adapter](x)
            after_A = self.lora_dropout[self.active_adapter](after_A)
